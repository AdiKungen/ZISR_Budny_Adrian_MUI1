...
public class MainClassifier {
	...
	public static void main(String[] args) {
		...
		int lSize = 100; //number of learning samples for each class

		// 1. Load data
		DataPackage all = new DataPackage();
		all.setFieldSeparator(";"); // default ,
		all.setDecimalSeparator('.'); // default .
		all.loadTextFile("data.txt");

		// divide classes
                DataPackage class1 = all.removeVectorsByColumnValue(36, 1);
                DataPackage class2 = all.removeVectorsByColumnValue(36, 2);
                DataPackage class3 = all.removeVectorsByColumnValue(36, 3);
		...
		for (DataVector vector : learnPkg.getList()) {
                    ...
                    if (temp > maxResult) { //store max result and class
                       maxResult = temp;
                       resultClass = vector.get(36); //first col stores the class no
                       resultVector = vector;
                    }
		}
		...
		//show results
                ...
                System.out.println("Real class: " + testVector.get(36));
